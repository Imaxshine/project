...
if($kisw<0 || $his<0 || $eng<0 || $scie<0 || $geo<0 || $histry<0 || $civic<0){
    ...
    $error=mysqli_query($conn,"SELECT * FROM class5 WHERE pupil='$mwanafunzi' AND parent='$mzazi'");
if(mysqli_num_rows($error)==0){
    $sql=$conn->prepare("INSERT INTO class5 (uniqid,pupil,parent,surname,gender,kisw,his,eng,scie,geo,histry,civic) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
    $sql->bind_param("issssiiiiiii",$uniqid,$mwanafunzi,$mzazi,$surname,$gender1,$kisw,$his,$eng,$scie,$geo,$histry,$civic);
    if($sql->execute()){
