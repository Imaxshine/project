...
if(isset($_POST['submit'])){
    ...
    if($kisw<0 || $mathe<0 || $eng<0 || $scie<0 || $geo<0 || $histry<0 || $develop<0){
    echo "Such of your marks is lowest than minmum marks (0).";
}elseif($kisw>100 || $mathe>100 || $eng>100 || $scie>100 || $geo>100 || $histry>100 || $develop>100){
    echo "Such of your marks is exceed maxmum (100) Marks. Please re-write again";
}else{
        
        if($gender=='FEMALE' || $gender=='MALE'){
     $sql=$conn->prepare("UPDATE class3 SET pupil=?,parent=?,surname=?,gender=?,kisw=?,his=?,eng=?,scie=?,geo=?,histry=?,develop=? WHERE id=?");
    $sql->bind_param("ssssiiiiiiii",$mwanafunzi,$mzazi,$surname,$gender2,$kisw,$mathe,$eng,$scie,$geo,$histry,$develop,$id);
    if($sql->execute()){
