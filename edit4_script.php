...
if(isset($_POST['submit'])){
    ...
    echo "Such of your marks is exceed maxmum (100) Marks. Please re-write again";
     ...
     $sql=$conn->prepare("UPDATE class4 SET pupil=?,parent=?,surname=?,gender=?,kisw=?,his=?,eng=?,scie=?,geo=?,histry=? WHERE uniqid=?");
    $sql->bind_param("ssssiiiiiii",$mwanafunzi,$mzazi,$surname,$gender2,$kisw,$mathe,$eng,$scie,$geo,$histry,$id);
    if($sql->execute()){
