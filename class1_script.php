...
if($kisw<0 || $his<0 || $eng<0 || $scie<0 || $geo<0 || $histry<0){
    ...
    $error=mysqli_query($conn,"SELECT * FROM subjects WHERE pupil='$mwanafunzi' AND parent='$mzazi'");
if(mysqli_num_rows($error)==0){
    $sql=$conn->prepare("INSERT INTO subjects (uniqid,pupil,parent,surname,gender,kisw,his,eng,scie,geo,histry) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
    $sql->bind_param("sssssiiiiii",$uniq_id,$mwanafunzi,$mzazi,$surname,$gender1,$kisw,$his,$eng,$scie,$geo,$histry);
    if($sql->execute()){
